...
describe('getInstallationInfo', () => {
  ...
  it('should detect global pnpm installation', async () => {
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe(`pnpm add -g ${defaultPackageName}@latest`);
  ...
  it('should detect global yarn installation', async () => {
    ...
    expect(info.updateCommand).toBe(
      `yarn global add ${defaultPackageName}@latest`,
    );
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect global bun installation', async () => {
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe(`bun add -g ${defaultPackageName}@latest`);
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should default to global npm installation for unrecognized paths', async () => {
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe(
      `npm install -g ${defaultPackageName}@latest`,
    );
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should use fork package name for update commands', async () => {
    ...
    expect(info.packageManager).toBe(PackageManager.NPM);
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe(`npm install -g ${forkPackageName}@latest`);
  });

  it('should use fork package name for pnpm update commands', async () => {
    const forkPackageName = '@mmmbuto/gemini-cli-termux';
    mockedGetPackageJson.mockResolvedValue({
      name: forkPackageName,
      version: '0.24.2-termux',
    ...
    const info = await getInstallationInfo(projectRoot, false);
    expect(info.packageManager).toBe(PackageManager.PNPM);
    expect(info.updateCommand).toBe(`pnpm add -g ${forkPackageName}@latest`);
  });

  it('should use fork package name for yarn update commands', async () => {
    const forkPackageName = '@mmmbuto/gemini-cli-termux';
    mockedGetPackageJson.mockResolvedValue({
      name: forkPackageName,
      version: '0.24.2-termux',
    ...
    expect(info.packageManager).toBe(PackageManager.YARN);
    expect(info.updateCommand).toBe(
      `yarn global add ${forkPackageName}@latest`,
    );
  });

  it('should use fork package name for bun update commands', async () => {
    const forkPackageName = '@mmmbuto/gemini-cli-termux';
    mockedGetPackageJson.mockResolvedValue({
      name: forkPackageName,
      version: '0.24.2-termux',
    ...
    const info = await getInstallationInfo(projectRoot, false);
    expect(info.packageManager).toBe(PackageManager.BUN);
    expect(info.updateCommand).toBe(`bun add -g ${forkPackageName}@latest`);
  });

  it('should fallback to default package name when getPackageJson fails', async () => {
